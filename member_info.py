    ...
    def insert_db(self,events=[]):
        ...
        cur.execute("UPDATE members SET school_id = %s, first_name = %s, last_name = %s, username=%s, \
            password=%s, gender=%s, member_age=%s, ethnicity=%s, continuing_new = %s, \
             passport_number=%s, passport_date_issued=%s, previous = %s, ethnicity_info=%s, teaching_research=%s, \
            ...
            self.age,self.ethnicity,self.mtype, self.passport,  self.date, self.previous,self.eth_info, self.research, self.promos, 
