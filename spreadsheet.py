...
def gen_mem_temp(schoolid, filetype):
    ...
    templatePath = os.path.join(
        basepath, 'downloads', 'End year template.xlsx')
    bg = op.load_workbook(templatePath)
    sheet1 = bg['Sheet1']
    sheet2 = bg['Username and passwords']
    ...
    cur = db.getCursor()
    cur.execute(
        "SELECT school_name FROM schools WHERE school_id = %s;", (schoolid,))
    sch_name = cur.fetchone()[0]
...
def gen_school_tem(schoolid,filetype):
    # get template object
    basepath = os.path.dirname(__file__)
    templatePath = os.path.join(
        basepath, 'downloads', 'School membership database.xlsx')
    bg = op.load_workbook(templatePath)
    sheet1 = bg['2021 active list']
    sheet2 = bg['Waiting list']
    ...
    # get school information from databse and insert into spreadsheet
    sheet1.cell(column=4, row=1, value=sch_name.capitalize())
    sheet2.cell(column=1, row=1, value=sch_name.capitalize())
    sql = "SELECT school_id, school_name, who, council, category, council, category, status, year, returning_number, total, max_num_2021, confirmed_num, coordinator_id, training, launch, passport_presentation, portal, passports, agreement, consent, notes FROM schools WHERE school_id = %s ORDER BY school_id" % schoolid
    cur.execute(sql)
    school_infos = cur.fetchall()
    if school_infos:
        for i in range(0, len(school_infos)):
            school_info = school_infos[i][0:-2]
            for j in range(0, len(school_info)):
                sheet1.cell(column=j+1, row=7+i, value=school_info[j])
            u_p = list(school_infos[i][1:3])+list(school_infos[i][-2:])
            for k in range(0, len(u_p)):
                sheet2.cell(column=k+1, row=7+i, value=u_p[k])
>>>>>>> Stashed changes
